...
const AccessibilityChoropleth = ({ width, height, data }) => {
  ...
  useEffect(() => {
    ...
    const colorScale = d3.scaleOrdinal([1,2,3], ["darkorange", "#98806B", "white"])

    const typology_map = {
        'Predominantly urban region': true,
        'Intermediate region, close to a city': true,
        'Predominantly rural region, close to a city': true,
        'Intermediate region, remote': false,
        'Predominantly rural region, remote': false
    }
    const code_map = {
        '1': 'Live in cities',
            ...
            update => update
                .attr('fill', function(d,i) {
                    ...
                    if (element) {
                        const element = data.find(function(e) { return d.id === e.CODE; });
                        if (element) {
                            const typology = element['Urban-rural typology'];
                            if (typology_map[typology]) {
                                return "darkorange"
                            }
                        }
                        return "#98806B";
    ...
    .data(['City within 45 minutes', 'Longer than 45 minutes', 'No Data'])
    .enter()
    .append("text")
...
export default AccessibilityChoropleth;